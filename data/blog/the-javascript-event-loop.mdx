...
draft: true
summary: If JavaScript is a single-threaded language, how does asynchronous functions work?
images: []
layout: PostLayout
...
A few days ago, a colleague tapped my shoulder for a sanity check while he was writing a test for a feature he was implementing. We noticed some strange behaviour, and not long after did we find out it was because of JavaScript's magical _event loop_ and _callback queue_.

And so begins the rabbit hole. What the heck is the event loop? Callback queue who? How can JavaScript be asynchronous, if it's a single-threaded programming language?
...
![Progression of the call stack during the execution of the script](/static/images/blog/call_stack.png)
Simple enough, right?
...
![A visualization of all the structures involved in allowing asynchronous programming in JS](/static/images/blog/js_structures.png)
When a function is called, it gets pushed onto the call stack. If the function is asynchronous and we need to wait some amount of time to get a result from it, it is popped from the call stack and pushed into the libuv pool. While we wait _t_ time for the function in the libuv pool to finish, the process continues executing functions by pushing them onto the call stack.
Eventually, whenever the function in the libuv pool finishes executing, a callback from the function is added to the event queue. The event loop then waits for the call stack to be empty. Whenever the call stack is empty, the event loop will check the event queue and pass a function from the queue into the stack to be executed.
...
Since the timeout is 0, at first glance, you'd expect all the statements to execute as-is line-by-line. But, no. Because of how JavaScript handles asynchronous code, this will still function the same as in the earlier example. Even though the setTimeout has a duration of 0ms, it will still be added to the libuv pool and then the event queue, but since the event loop has to wait for the call stack to be empty before it can move something from the event queue to the call stack, the callback inside the setTimeout is still the last function executed, causing the output to be:
...
- The illusion of being multi-threaded is created by the event loop and the event queue.
- Asynchronous functions are handled by libuv, which pushes them into the event queue when they're done executing.
  - The event loop waits for the call stack to be empty before it moves a function from the event queue to the call stack
